...
<asp:GridView ID="gvEntity" runat="server" AutoGenerateColumns="False" Width="100%"
    ...
    <Columns>
        <asp:TemplateField HeaderText="類別">
            <ItemTemplate>
                <%# ((eIVOCenter.Helper.BusinessCounterpartUploadManager)_mgr).BusinessType.ToString() %></ItemTemplate>
            <ItemStyle HorizontalAlign="Center" />
        </asp:TemplateField>
        <asp:TemplateField HeaderText="營業人名稱">
            <ItemTemplate>
                <%# (((ItemUpload<Organization>)Container.DataItem)).Columns[0] %></ItemTemplate>
            <ItemStyle HorizontalAlign="Left" />
        </asp:TemplateField>
        <asp:TemplateField HeaderText="統一編號">
            <ItemTemplate>
                <%# (((ItemUpload<Organization>)Container.DataItem)).Columns[1]%></ItemTemplate>
            <ItemStyle HorizontalAlign="Center" />
        </asp:TemplateField>
        <asp:TemplateField HeaderText="聯絡人電子郵件">
            <ItemTemplate>
                <%# (((ItemUpload<Organization>)Container.DataItem)).Columns[2] %>
            </ItemTemplate>
            <ItemStyle HorizontalAlign="Left" />
        </asp:TemplateField>
        <asp:TemplateField HeaderText="地址" FooterText="總計">
            <ItemTemplate>
                <%# (((ItemUpload<Organization>)Container.DataItem)).Columns[3] %>
            </ItemTemplate>
            <FooterStyle HorizontalAlign="Right" />
        </asp:TemplateField>
        <asp:TemplateField HeaderText="電話">
            <ItemTemplate>
                <%# (((ItemUpload<Organization>)Container.DataItem)).Columns[4] %>
            </ItemTemplate>
            <FooterTemplate>
        ...
        <asp:TemplateField HeaderText="匯入狀態">
            <ItemTemplate>
                <font color="red"><%# String.Format("{0}{1}", (((ItemUpload<Organization>)Container.DataItem)).UploadStatus.ToString(),(((ItemUpload<Organization>)Container.DataItem)).Status) %></font>
            </ItemTemplate>
            <FooterTemplate>

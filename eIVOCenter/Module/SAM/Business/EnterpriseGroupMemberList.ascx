<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="EnterpriseGroupMemberList.ascx.cs"
    Inherits="eIVOCenter.Module.SAM.Business.EnterpriseGroupMemberList" %>
...
<%@ Register src="../../Entity/OrganizationEntrustingTokenView.ascx" tagname="OrganizationEntrustingTokenView" tagprefix="uc3" %>
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="企業狀態" SortExpression="CurrentLevel">
                <ItemTemplate>
                    ...
                    <%# ((EnterpriseGroupMember)Container.DataItem).Organization.OrganizationStatus.Entrusting==true ? "<br/>自動開立＼接收":"" %>
                    <a href="#" onclick="<%# doShowToken.GetPostBackEventReference(((EnterpriseGroupMember)Container.DataItem).CompanyID.ToString()) %>"><%# ((EnterpriseGroupMember)Container.DataItem).Organization.OrganizationStatus.TokenID.HasValue ? "(檢視憑證)" : ""%></a>
                    <%# ((EnterpriseGroupMember)Container.DataItem).Organization.OrganizationStatus.Entrusting==true && !((EnterpriseGroupMember)Container.DataItem).Organization.OrganizationStatus.TokenID.HasValue ? "(憑證未設定)":"" %>
                </ItemTemplate>
            </asp:TemplateField>
...
<uc1:ActionHandler ID="doShowToken" runat="server" />
...
<uc3:OrganizationEntrustingTokenView ID="tokenView" Visible="false" runat="server" />
<script runat="server">
    protected override void OnInit(EventArgs e)
    {
        ...
        doShowToken.DoAction = arg =>
            {
                tokenView.QueryExpr = o => o.CompanyID == int.Parse(arg);
                tokenView.BindData();
            };

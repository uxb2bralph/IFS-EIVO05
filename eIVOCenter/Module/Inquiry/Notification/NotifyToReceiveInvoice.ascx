...
<uc1:PageAction ID="actionItem" runat="server" 
    ItemName="下列資料已傳送至集團加值中心，請登入平台執行接收。" EnableIcon="False" />
<uc2:FunctionTitleBar ID="titleBar" runat="server" ItemName="電子發票" 
    EnableIcon="False" />
...
<script runat="server">
    protected override void OnInit(EventArgs e)
    {
        ...
        if (!String.IsNullOrEmpty(Request["businessID"]) && int.TryParse(Request["businessID"], out businessID))
        {
            ...
            {
                return table.Where(d => d.CurrentStep == (int)Naming.B2BInvoiceStepDefinition.待接收 && d.DocType == (int)Naming.B2BInvoiceDocumentTypeDefinition.電子發票
                    && d.DocumentDispatches.Where(r => r.TypeID == (int)Naming.B2BInvoiceDocumentTypeDefinition.電子發票).Count() > 0
                    && d.InvoiceItem.InvoiceBuyer.BuyerID == businessID);
            };

            invoiceCancelList.BuildQuery = table =>
            {
                return table.Where(d => d.CurrentStep == (int)Naming.B2BInvoiceStepDefinition.待接收 && d.DocType == (int)Naming.B2BInvoiceDocumentTypeDefinition.作廢發票
                    && d.DocumentDispatches.Where(r => r.TypeID == (int)Naming.B2BInvoiceDocumentTypeDefinition.作廢發票).Count() > 0
                    && d.DerivedDocument.ParentDocument.InvoiceItem.InvoiceBuyer.BuyerID == businessID);
            };

            allowanceList.BuildQuery = table =>
            {
                return table.Where(d => d.CurrentStep == (int)Naming.B2BInvoiceStepDefinition.待接收 && d.DocType == (int)Naming.B2BInvoiceDocumentTypeDefinition.發票折讓
                    && d.DocumentDispatches.Where(s => s.TypeID == (int)Naming.B2BInvoiceDocumentTypeDefinition.發票折讓).Count() > 0
                    && d.InvoiceAllowance.InvoiceAllowanceSeller.SellerID == businessID);
            };

            allowanceCancelList.BuildQuery = table =>
            {
                return table.Where(d => d.CurrentStep == (int)Naming.B2BInvoiceStepDefinition.待接收 && d.DocType == (int)Naming.B2BInvoiceDocumentTypeDefinition.作廢折讓
                    && d.DocumentDispatches.Where(s => s.TypeID == (int)Naming.B2BInvoiceDocumentTypeDefinition.作廢折讓).Count() > 0
                    && d.DerivedDocument.ParentDocument.InvoiceAllowance.InvoiceAllowanceSeller.SellerID == businessID);
            };

            receiptList.BuildQuery = table =>
            {
                return table.Where(d => d.CurrentStep == (int)Naming.B2BInvoiceStepDefinition.待接收 && d.DocType == (int)Naming.B2BInvoiceDocumentTypeDefinition.收據
                    && d.DocumentDispatches.Where(r => r.TypeID == (int)Naming.B2BInvoiceDocumentTypeDefinition.收據).Count() > 0
                    && d.ReceiptItem.BuyerID == businessID);
            };

            receiptCancelList.BuildQuery = table =>
            {
                return table.Where(d => d.CurrentStep == (int)Naming.B2BInvoiceStepDefinition.待接收 && d.DocType == (int)Naming.B2BInvoiceDocumentTypeDefinition.作廢收據
                    && d.DocumentDispatches.Where(r => r.TypeID == (int)Naming.B2BInvoiceDocumentTypeDefinition.作廢收據).Count() > 0
                    && d.DerivedDocument.ParentDocument.ReceiptItem.BuyerID == businessID);
            };                                             

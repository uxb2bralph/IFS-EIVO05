    ...
    <asp:GridView ID="gvEntity" runat="server" AutoGenerateColumns="False" Width="100%"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="開立發票折讓營業人">
                <ItemTemplate>
                    <%# ((CDS_Document)Container.DataItem).DerivedDocument.ParentDocument.InvoiceAllowance.InvoiceAllowanceBuyer.CustomerName%>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
            </asp:TemplateField>
            <asp:TemplateField HeaderText="接收發票折讓營業人">
                <ItemTemplate>
                    <%# ((CDS_Document)Container.DataItem).DerivedDocument.ParentDocument.InvoiceAllowance.InvoiceAllowanceSeller.CustomerName%>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
            </asp:TemplateField>
            <asp:TemplateField HeaderText="折讓單號碼" FooterText="總計金額">
                <ItemTemplate>
                    <%# ((CDS_Document)Container.DataItem).DerivedDocument.ParentDocument.InvoiceAllowance.AllowanceNumber%>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />

    ...
    <asp:GridView ID="gvEntity" runat="server" AutoGenerateColumns="False" Width="100%"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="開立發票營業人">
                <ItemTemplate>
                    <%# ((CDS_Document)Container.DataItem).DerivedDocument.ParentDocument.InvoiceItem.InvoiceSeller.CustomerName %>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Left" />
            </asp:TemplateField>
            <asp:TemplateField HeaderText="接收發票營業人">
                <ItemTemplate>
                    <%# ((CDS_Document)Container.DataItem).DerivedDocument.ParentDocument.InvoiceItem.InvoiceBuyer.CustomerName %>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Left" />
            </asp:TemplateField>
            <asp:TemplateField HeaderText="發票號碼">
                <ItemTemplate>
                    <%# ((CDS_Document)Container.DataItem).DerivedDocument.ParentDocument.InvoiceItem.TrackCode%><%# ((CDS_Document)Container.DataItem).DerivedDocument.ParentDocument.InvoiceItem.No %>
                </ItemTemplate>
            ...
            <asp:TemplateField HeaderText="發票狀態">
                <ItemTemplate>
                    <%# ((Naming.B2BInvoiceStepDefinition)((CDS_Document)Container.DataItem).CurrentStep).ToString() %>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
                <FooterTemplate>

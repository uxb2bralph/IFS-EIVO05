    ...
    <asp:GridView ID="gvEntity" runat="server" AutoGenerateColumns="False" Width="100%"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="列印狀態">
                <ItemTemplate>
                    <%# ((CDS_Document)Container.DataItem).DocumentPrintLogs.Any(l => l.TypeID == (int)Model.Locale.Naming.DocumentTypeDefinition.E_Allowance) ? "已列印" : "未列印"%>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
            </asp:TemplateField>
            <asp:TemplateField HeaderText="主動列印">
                <ItemTemplate>
                    <%# ((CDS_Document)Container.DataItem).InvoiceAllowance.InvoiceAllowanceBuyer.Organization.OrganizationStatus.EntrustToPrint == null ? "否" : (bool)((CDS_Document)Container.DataItem).InvoiceAllowance.InvoiceAllowanceBuyer.Organization.OrganizationStatus.EntrustToPrint ? "是" : "否"%>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
            </asp:TemplateField>
        </Columns>

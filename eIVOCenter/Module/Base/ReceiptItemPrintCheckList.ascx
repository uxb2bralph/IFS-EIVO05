...
<%@ Register Src="~/Module/EIVO/Action/ReceiptItemCommonView.ascx" TagName="ReceiptItemCommonView"
    TagPrefix="uc6" %>
    ...
    <asp:GridView ID="gvEntity" runat="server" AutoGenerateColumns="False" Width="100%"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="對帳單">
                <ItemTemplate>
                    <a href="#" onclick="<%# getStatement((CDS_Document)Container.DataItem) %>">
                        <asp:Image ID="Image1" BorderStyle="None" ImageUrl="~/images/icon_ca.gif" runat="server"
                            Visible="<%# ((CDS_Document)Container.DataItem).Attachment.Count<=0 ? false : true %>" />
                    </a>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
            </asp:TemplateField>
            <asp:TemplateField HeaderText="金額" FooterText="總計金額">
                <ItemTemplate>
                    <%#String.Format("{0:0,0.00}", ((CDS_Document)Container.DataItem).ReceiptItem.TotalAmount)%></ItemTemplate>
            ...
            <asp:TemplateField HeaderText="列印狀態">
                <ItemTemplate>
                    <%--<asp:Label ID="Printstatus" runat="server" Text="<%# checkPrintStatus(((CDS_Document)Container.DataItem).ReceiptItem.CDS_Document) %>"
                        EnableViewState="false"></asp:Label>--%>
                    <%# ((CDS_Document)Container.DataItem).DocumentPrintLogs.Any(l => l.TypeID == (int)Model.Locale.Naming.DocumentTypeDefinition.E_Receipt) ? "已列印" : ((CDS_Document)Container.DataItem).DocumentPrintQueue != null ? "已列印" : "未列印"%>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
            </asp:TemplateField>
        </Columns>
...
<script runat="server">
    ...
    //DocumentPrintLogs中若有資料,則表示收據"已列印" (若為收據列印功能,則表格的最右邊需顯示"列印狀態"欄位)
    private string checkPrintStatus(CDS_Document document)
    {
        return document.DocumentPrintLogs.Any(l => l.TypeID == (int)Model.Locale.Naming.DocumentTypeDefinition.E_Receipt) ? "已列印" : "未列印";
    }

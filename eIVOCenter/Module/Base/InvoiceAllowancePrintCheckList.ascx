    ...
    <asp:GridView ID="gvEntity" runat="server" AutoGenerateColumns="False" Width="100%"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="金額" FooterText="總計金額">
                <ItemTemplate>
                    <%#String.Format("{0:0,0.00}", ((CDS_Document)Container.DataItem).InvoiceAllowance.TotalAmount)%></ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
                <FooterStyle HorizontalAlign="Right" />
            </asp:TemplateField>
            ...
            <asp:TemplateField HeaderText="列印狀態">
                <ItemTemplate>
                    <%# ((CDS_Document)Container.DataItem).DocumentPrintLogs.Any(l => l.TypeID == (int)Model.Locale.Naming.DocumentTypeDefinition.E_Allowance) ? "已列印" : ((CDS_Document)Container.DataItem).DocumentPrintQueue != null ? "已列印" : "未列印"%>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
            </asp:TemplateField>
        </Columns>

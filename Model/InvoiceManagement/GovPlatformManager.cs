...
{
    public class GovPlatformManager
    {
        public GovPlatformManager()
        { 
        }
...
//                var invItems = replication.Where(r => r.TypeID == (int)Naming.DocumentTypeDefinition.E_Invoice && !r.LastActionTime.HasValue).Select(r => r.CDS_Document.InvoiceItem);
                ...
                if (invItems.Count() > 0)
                {
                    ...
                    //    {
                    //        writeInvoiceItem(fs, item);
                    //        replication.DeleteOnSubmit(item.CDS_Document.DocumentReplication);
                    //    }
                    //}
...
//                var cancelItems = replication.Where(r => r.TypeID == (int)Naming.DocumentTypeDefinition.E_InvoiceCancellation && !r.LastActionTime.HasValue).Select(r => r.CDS_Document.InvoiceItem);
                ...
                if (cancelItems.Count() > 0)
                {
                    ...
                    //    {
                    //        writeInvoiceCancellation(fs, item);
                    //        replication.DeleteOnSubmit(item.CDS_Document.DocumentReplication);
                    //    }
                    //}
                ...
                saveInvoiceForGovPlatform(mgr);
                saveInvoiceCancellationForGovPlatform(mgr);
                saveAllwanceForGovPlatform(mgr);
                saveAllowancCancellationForGovPlatform(mgr);

            }
        }

        private void saveInvoiceForGovPlatform(InvoiceManager mgr)
        {
            Settings.Default.A0401Outbound.CheckStoredPath();

            int counter = Directory.GetFiles(Settings.Default.A0401Outbound).Length;
            var table = mgr.GetTable<DocumentDispatch>();
            var items = table.Where(d => d.TypeID == (int)Naming.DocumentTypeDefinition.E_Invoice);

            if (items.Count() > 0)
            {
                foreach (var item in items)
                {
                    String fileName = Path.Combine(Settings.Default.A0401Outbound, String.Format("A0401-{0:yyyyMMddHHmmssf}-{1:00000}.xml", DateTime.Now, counter++));
                    item.CDS_Document.InvoiceItem.BuildA0101().ConvertToXml().Save(fileName);
                }
        ...
        private void saveAllwanceForGovPlatform(InvoiceManager mgr)
        {
            Settings.Default.B0401Outbound.CheckStoredPath();

            int counter = Directory.GetFiles(Settings.Default.B0401Outbound).Length;
            var table = mgr.GetTable<DocumentDispatch>();
            var items = table.Where(d => d.TypeID == (int)Naming.DocumentTypeDefinition.E_Allowance);

            if (items.Count() > 0)
            {
                foreach (var item in items)
                {
                    String fileName = Path.Combine(Settings.Default.B0401Outbound, String.Format("B0401-{0:yyyyMMddHHmmssf}-{1:00000}.xml", DateTime.Now, counter++));
                    item.CDS_Document.InvoiceAllowance.BuildB0101().ConvertToXml().Save(fileName);
                }
                table.DeleteAllOnSubmit(items);
                mgr.SubmitChanges();
            }
        ...
        private void saveInvoiceCancellationForGovPlatform(InvoiceManager mgr)
        {
            Settings.Default.A0501Outbound.CheckStoredPath();

            int counter = Directory.GetFiles(Settings.Default.A0501Outbound).Length;
            var table = mgr.GetTable<DocumentDispatch>();
            var items = table.Where(d => d.TypeID == (int)Naming.DocumentTypeDefinition.E_InvoiceCancellation);

            if (items.Count() > 0)
            {
                foreach (var item in items)
                {
                    String fileName = Path.Combine(Settings.Default.A0501Outbound, String.Format("A0501-{0:yyyyMMddHHmmssf}-{1:00000}.xml", DateTime.Now, counter++));
                    item.CDS_Document.DerivedDocument.ParentDocument.InvoiceItem.BuildA0201().ConvertToXml().Save(fileName);
                }
                table.DeleteAllOnSubmit(items);
                mgr.SubmitChanges();
            }
        ...
        private void saveAllowancCancellationForGovPlatform(InvoiceManager mgr)
        {
            Settings.Default.B0501Outbound.CheckStoredPath();

            int counter = Directory.GetFiles(Settings.Default.B0501Outbound).Length;
            var table = mgr.GetTable<DocumentDispatch>();
            var items = table.Where(d => d.TypeID == (int)Naming.DocumentTypeDefinition.E_AllowanceCancellation);

            if (items.Count() > 0)
            {
                foreach (var item in items)
                {
                    String fileName = Path.Combine(Settings.Default.B0501Outbound, String.Format("B0501-{0:yyyyMMddHHmmssf}-{1:00000}.xml", DateTime.Now, counter++));
                    item.CDS_Document.DerivedDocument.ParentDocument.InvoiceAllowance.BuildB0201().ConvertToXml().Save(fileName);
                }
                table.DeleteAllOnSubmit(items);
                mgr.SubmitChanges();
            }

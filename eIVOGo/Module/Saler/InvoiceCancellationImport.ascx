<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="InvoiceCancellationImport.ascx.cs"
    Inherits="eIVOGo.Module.Saler.InvoiceCancellationImport" %>
        ...
        <asp:GridView ID="gvEntity" runat="server" EnableViewState="True" CssClass="table01"
            ...
            <Columns>
                <asp:TemplateField HeaderText="序號">
                    <ItemTemplate>
                        <%#  ((InvoiceCancellations)Container.DataItem).OrderNo %>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="goolge id">
                    <ItemTemplate>
                        <asp:Label ID="lable2" runat="server" Text='<%# ((InvoiceCancellations)Container.DataItem).CustomerID %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="發票號碼">
                    <ItemTemplate>
                        <asp:Label ID="lable3" runat="server" Text='<%#  ((InvoiceCancellations)Container.DataItem).No %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="作廢原因">
                    <ItemTemplate>
                        <asp:Label ID="lable4" runat="server" Text='<%#  ((InvoiceCancellations)Container.DataItem).Remark %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="買受人名稱">
                    <ItemTemplate>
                        <asp:Label ID="lable5" runat="server" Text='<%# ((InvoiceCancellations)Container.DataItem).BuyerName %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="買受人統編">
                    <ItemTemplate>
                        <asp:Label ID="lable6" runat="server" Text='<%# ((InvoiceCancellations)Container.DataItem).BuyerReceiptNo %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="未稅金額">
                    <ItemTemplate>
                        <asp:Label ID="lable7" runat="server" Text='<%# ((InvoiceCancellations)Container.DataItem).SalesAmount.ToString () == "0" ? "N/A" : String.Format("{0:##,###,###,##0.00}", ((InvoiceCancellations)Container.DataItem).SalesAmount)%>' />
                    </ItemTemplate>
                ...
                <asp:TemplateField HeaderText="稅額">
                    <ItemTemplate>
                        <asp:Label ID="lable7" runat="server" Text='<%# ((InvoiceCancellations)Container.DataItem).TaxAmount.ToString () == "0" ? "N/A" : String.Format("{0:##,###,###,##0.00}", ((InvoiceCancellations)Container.DataItem).TaxAmount)%>' />
                    </ItemTemplate>
                    <ItemStyle HorizontalAlign="Right" />
                    <FooterStyle HorizontalAlign="Right" />
                </asp:TemplateField>
                <asp:TemplateField HeaderText="含稅金額">
                    <ItemTemplate>
                        <asp:Label ID="lable7" runat="server" Text='<%# ((InvoiceCancellations)Container.DataItem).invAmount.ToString () == "0" ? "N/A" : String.Format("{0:##,###,###,##0.00}", ((InvoiceCancellations)Container.DataItem).invAmount)%>' />
                    </ItemTemplate>
                    <ItemStyle HorizontalAlign="Right" />
                    <FooterStyle HorizontalAlign="Right" />
                    <FooterTemplate>
                ...
                <asp:TemplateField HeaderText="滙入狀態">
                    <ItemTemplate>
                        ...
                        <asp:Label ID="lable8" runat="server" Text='<%#  ((InvoiceCancellations)Container.DataItem).Status %>' />
                        <%# "</font >" %>
                    </ItemTemplate>
                    <ItemStyle HorizontalAlign="Center" />
                    <FooterStyle HorizontalAlign="Right" />
                    <FooterTemplate>
                        <asp:Label ID="Label9" runat="server" Text='<%# ("成功:"+ YesNum.ToString() + "  失敗:" + NoNum.ToString()) %>' />
                    </FooterTemplate>
                </asp:TemplateField>
            </Columns>
            <PagerTemplate>

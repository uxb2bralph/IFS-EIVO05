<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="InvoiceAllowanceImport.ascx.cs"
    Inherits="eIVOGo.Module.Saler.InvoiceAllowanceImport" %>
...
<!--路徑名稱-->
<uc5:PageAction ID="actionItem" runat="server" ItemName="首頁 >折讓單滙入" />
<!--交易畫面標題-->
<uc6:FunctionTitleBar ID="titleBar" runat="server" ItemName="折讓單滙入" />
<div id="border_gray">
    <!--表格 開始-->
        ...
        <asp:GridView ID="gvEntity" runat="server" EnableViewState="True" CssClass="table01"
            ...
            <Columns>
                <asp:TemplateField HeaderText="序號">
                    <ItemTemplate>
                        <%#  ((InvoiceAllowances)Container.DataItem).OrderNo%>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="goolge id">
                    <ItemTemplate>
                        <asp:Label ID="lable2" runat="server" Text='<%# ((InvoiceAllowances)Container.DataItem).CustomerID %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="折讓號碼">
                    <ItemTemplate>
                        <asp:Label ID="lable3" runat="server" Text='<%#  ((InvoiceAllowances)Container.DataItem).AllowanceNo %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="原發票號碼">
                    <ItemTemplate>
                        <asp:Label ID="lable3" runat="server" Text='<%#  ((InvoiceAllowances)Container.DataItem).InvNo %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="買受人名稱">
                    <ItemTemplate>
                        <asp:Label ID="lable5" runat="server" Text='<%# ((InvoiceAllowances)Container.DataItem).BuyerCustomerName %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="買受人統編">
                    <ItemTemplate>
                        <asp:Label ID="lable6" runat="server" Text='<%# ((InvoiceAllowances)Container.DataItem).BuyerReceiptNo %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="未稅金額">
                    <ItemTemplate>
                        <asp:Label ID="lable7" runat="server" Text='<%# ((InvoiceAllowances)Container.DataItem).Amount.ToString () == "0" ? "N/A" : String.Format("{0:##,###,###,##0.00}", decimal.Parse (((InvoiceAllowances)Container.DataItem).Amount))%>' />
                    </ItemTemplate>
                ...
                <asp:TemplateField HeaderText="稅額">
                    <ItemTemplate>
                        <asp:Label ID="lable7" runat="server" Text='<%# ((InvoiceAllowances)Container.DataItem).Tax.ToString () == "0" ? "N/A" : String.Format("{0:##,###,###,##0.00}", decimal.Parse (((InvoiceAllowances)Container.DataItem).Tax))%>' />
                    </ItemTemplate>
                    <ItemStyle HorizontalAlign="Right" />
                    <FooterStyle HorizontalAlign="Right" />
                </asp:TemplateField>
                <asp:TemplateField HeaderText="含稅金額">
                    <ItemTemplate>
                        <asp:Label ID="lable7" runat="server" Text='<%# ((InvoiceAllowances)Container.DataItem).TAmount.ToString () == "0" ? "N/A" : String.Format("{0:##,###,###,##0.00}", decimal.Parse (((InvoiceAllowances)Container.DataItem).TAmount))%>' />
                    </ItemTemplate>
                    <ItemStyle HorizontalAlign="Right" />
                    <FooterStyle HorizontalAlign="Right" />
                    <FooterTemplate>
                ...
                <asp:TemplateField HeaderText="滙入狀態">
                    <ItemTemplate>
                        ...
                        <asp:Label ID="lable8" runat="server"  Text='<%#  ((InvoiceAllowances)Container.DataItem).Status %>' ForeColor="#FF3300" Font-Size="Small" />
                        <%# "</font >" %>
                    </ItemTemplate>
